...
parser.add_argument('--epoch', type=int, default=175, help='Epoch to run [default: 50]')
...
# # Tianxu: take part number of each object category (not used)
# obj_part_num_file = os.path.join(hdf5_data_dir, 'object_part_num.txt')
# with open(obj_part_num_file, 'r') as f:
#   obj_part_num = [int(line.rstrip().split(' ')[1]) for line in f.readlines()]

all_cats = json.load(open(os.path.join(hdf5_data_dir, 'overallid_to_catid_partid.json'), 'r'))
NUM_CATEGORIES = 16

# Tianxu: Change number of parts to 6, which is the max possible number of part for an instance (motorcycle)
####################################
      ...
      for i in range(FLAGS.num_gpu):
        with tf.device('/gpu:%d' % i):
          with tf.name_scope('%s_%d' % (TOWER_NAME, i)) as scope:
            ...
            # Tianxu: permutation invariant loss
            ###########################################################################
            # loss, per_instance_seg_loss, per_instance_seg_pred_res  \
            #   = model.get_loss(seg_pred, seg_phs[-1])

            pi_loss, perm_labels = model.get_permutation_invariant_loss(seg_pred, seg_phs[-1])
            
            # pi_loss should be the same with loss
            loss, per_instance_seg_loss, per_instance_seg_pred_res  \
              = model.get_loss(seg_pred, perm_labels)
    ...
    def train_one_epoch(train_file_idx, epoch_num):
      ...
      for i in range(num_train_file):
        ...
        for j in range(num_batch):
          ...
          # Tianxu: use permuted_labels as the new ground truth
          ########################################################################################################################
          # train_op is for both gpus, and the others are for gpu_1
          _, loss_val, per_instance_seg_loss_val, seg_pred_val, pred_seg_res, permuted_labels \
              = sess.run([train_op, loss, per_instance_seg_loss, seg_pred, per_instance_seg_pred_res, perm_labels], \
              feed_dict=feed_dict)

          #per_instance_part_acc = np.mean(pred_seg_res == cur_seg[begidx_1: endidx_1, ...], axis=1)
          per_instance_part_acc = np.mean(pred_seg_res == permuted_labels, axis=1)
          ########################################################################################################################
    ...
    def eval_one_epoch(epoch_num):
      ...
      for i in range(num_test_file):
        ...
        for j in range(num_batch):
          ...
          # Tianxu: use permuted_labels as the new ground truth
          #############################################################################################################
          loss_val, per_instance_seg_loss_val, seg_pred_val, pred_seg_res, permuted_labels \
              = sess.run([loss, per_instance_seg_loss, seg_pred, per_instance_seg_pred_res, perm_labels], \
              feed_dict=feed_dict)
