...
parser.add_argument('--model_path', default=os.path.join(BASE_DIR, './train_results/trained_models/epoch_175.ckpt'), help='Model checkpoint path')
...
def load_h5_data(h5_file_path, num_points):
    data = []
    label = []
    ...
    return np.array(data, dtype=np.float32), np.array(label, dtype=np.uint8)
...
def predict():
  ...
  with tf.Session(config=config) as sess:
    ...
    cur_label_one_hot = np.zeros((1, NUM_OBJ_CATS), dtype=np.float32) # dummy info for pure part segmentation without cat info

    sapien_h5_files = os.listdir(sapien_h5_dir)
    for h5_file_path in sapien_h5_files:
      pts, seg = load_h5_data(os.path.join(sapien_h5_dir, h5_file_path), point_num)
    ...
    #########################################################################################################
      ...
      if output_verbose:
        output_color_point_cloud(pts, seg, os.path.join(output_dir, str(h5_file_path)+'_gt.obj'))
        output_color_point_cloud(pts, seg_pred_val, os.path.join(output_dir, str(h5_file_path)+'_pred.obj'))
        output_color_point_cloud_red_blue(pts, np.int32(seg == seg_pred_val), 
            os.path.join(output_dir, str(h5_file_path)+'_diff.obj'))

        with open(os.path.join(output_dir, str(h5_file_path)+'.log'), 'w') as fout:
