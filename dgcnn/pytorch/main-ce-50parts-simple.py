...
def _init_():
    if not os.path.exists('checkpoints_ce_50_simple'):
        os.makedirs('checkpoints_ce_50_simple')
    if not os.path.exists('checkpoints_ce_50_simple/'+args.exp_name):
        os.makedirs('checkpoints_ce_50_simple/'+args.exp_name)
    if not os.path.exists('checkpoints_ce_50_simple/'+args.exp_name+'/'+'models'):
        os.makedirs('checkpoints_ce_50_simple/'+args.exp_name+'/'+'models')
    os.system('cp main.py checkpoints_ce_50_simple'+'/'+args.exp_name+'/'+'main.py.backup')
    os.system('cp model.py checkpoints_ce_50_simple' + '/' + args.exp_name + '/' + 'model.py.backup')
    os.system('cp util.py checkpoints_ce_50_simple' + '/' + args.exp_name + '/' + 'util.py.backup')
    os.system('cp data.py checkpoints_ce_50_simple' + '/' + args.exp_name + '/' + 'data.py.backup')
...
def train(args, io):
    ...
    if args.model == 'pointnet':
        ...
        model = DGCNN(args, part_number=50).to(device)
    else:
    ...
    starting_epoch = 0
    training_backup_filepath = F'checkpoints_ce_50_simple/{args.exp_name}/models/training_backup.txt'
    ...
    for epoch in range(starting_epoch, args.epochs):
        ...
        if test_acc > max_test_acc:
            ...
            max_acc_epoch = epoch
            torch.save(model.state_dict(), 'checkpoints_ce_50_simple/%s/models/model.h5' % args.exp_name)
    ...
    acc_ax.legend([F'train', \
                F'test'], loc='upper right')
    #plt.show()
    fig.savefig('./log_ce_50_simple/model_loss_acc.png')
...
if __name__ == "__main__":
    ...
    io = IOStream('checkpoints_ce_50_simple/' + args.exp_name + '/run.log')
    io.cprint(str(args))

    args.use_cuda = not args.no_cuda and torch.cuda.is_available()
    args.model_path = os.path.join('checkpoints_ce_50_simple', args.exp_name, 'models', 'model.h5')
    ...
    args.eval = False
    if not args.eval:
        if not os.path.exists('./log_ce_50_simple'):
            os.mkdir('./log_ce_50_simple')
        train(args, io)
    else:
        test(args, io)
