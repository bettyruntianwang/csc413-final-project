...
def get_graph_feature(x, k=20, idx=None, use_cuda=True):
    batch_size = x.size(0)
    num_points = x.size(2)
...
class DGCNN(nn.Module):
    def __init__(self, args, part_number=6):
        ...
        self.linear3 = nn.Linear(256, part_number)
...
class DGCNN_classification(nn.Module):
    def __init__(self, args, output_channels=40):
        super(DGCNN_classification, self).__init__()
        self.args = args
        self.k = args.k
        self.cuda = args.cuda
    ...
    def forward(self, x):
        batch_size = x.size(0)
        x = get_graph_feature(x, k=self.k, use_cuda=self.cuda)
        x = self.conv1(x)
        x1 = x.max(dim=-1, keepdim=False)[0]

        x = get_graph_feature(x1, k=self.k, use_cuda=self.cuda)
        x = self.conv2(x)
        x2 = x.max(dim=-1, keepdim=False)[0]

        x = get_graph_feature(x2, k=self.k, use_cuda=self.cuda)
        x = self.conv3(x)
        x3 = x.max(dim=-1, keepdim=False)[0]

        x = get_graph_feature(x3, k=self.k, use_cuda=self.cuda)
        x = self.conv4(x)
        x4 = x.max(dim=-1, keepdim=False)[0]
